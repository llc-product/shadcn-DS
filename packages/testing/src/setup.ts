// @digitaltwin/testing/setup — the jsdom stubs every React suite in this org needs.
//
// Moved out of packages/ui so a consuming app gets the same stubs rather than a copy that drifts.
// Every assignment uses `??=` so a suite that needs a real implementation can install one first
// and keep it, and so this file stays safe to load from a `@vitest-environment node` suite.
